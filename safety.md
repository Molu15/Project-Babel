...
class InputObserver:
    ...
    def _inject_zoom(self, delta):
        ...
        # Helper to simulate key states directly via Windows API
        def set_key_state(vk, down=True):
            # 0 = Down, 2 = Up
            flags = 0 if down else KEYEVENTF_KEYUP
            # keybd_event is deprecated but reliable for simple modifier injection
            user32.keybd_event(vk, 0, flags, 0)

        # 1. Force Release Control (Critical step to stop Panning)
        # We release it physically to ensure Photoshop stops seeing "Ctrl"
        set_key_state(VK_CONTROL, down=False)
        time.sleep(0.01)

        # 2. Press ALT (Hold it)
        set_key_state(VK_MENU, down=True)
        time.sleep(0.02) # Allow OS to register state change

        # 3. Send Scroll Event
        # The 'mouse' library is generally fine here if the modifiers are set correctly above
        import mouse
        steps = int(delta / 120.0)
        mouse.wheel(steps)
        
        # 4. Wait for app to process scroll
        time.sleep(0.02)
        
        # 5. Release Alt
        set_key_state(VK_MENU, down=False)
        
        # 6. Smart Restore Control
        # If the user is physically holding Ctrl, we must restore it logically
        # otherwise their Ctrl key will stop working until they press it again.
        if (user32.GetAsyncKeyState(VK_CONTROL) & 0x8000) != 0:
            set_key_state(VK_CONTROL, down=True)
